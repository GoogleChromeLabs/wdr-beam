...
export const caseHtmlClass = async (
  markdown: string,
  file: string,
  uid: string,
  imageCache: ImageCahce
): Promise<MarkdownResults> => {
  const imageMatchRegex = /<img+[^>]*class="(.*?)"+[^>]*src="((?!http?).*\\*.(png|svg|jpg|jpeg|gif|webp))"+[^>]*alt="(.*?)"[^>]*>/g;
  const imageRegexGroup = /<img+[^>]*class="(?<class>(.*?))"+[^>]*src="(?<src>((?!http?).*\\*.(png|svg|jpg|jpeg|gif|webp)))"+[^>]*alt="(?<alt>(.*?))"[^>]*>/;
    ...
    for (const image of images) {
      ...
      if (groups && groups.src) {
        ...
        if (fs.existsSync(srcPath)) {
          ...
          const shortcode = `{% Img src="${upload.src}", alt="${groups.alt}", width="${upload.width}", height="${upload.height}", class="${groups.class}" %}`;
